...
运行方式:
  julia --project=. benchmark/accuracy/run_P1_P3_pmchw.jl [P1] [P3]
  (不带参数时运行 P1)
"""
...
if "P1" in enabled
    ...
    @printf("  球半径: %.4f m\n", radius_m)
    mie_phi0  = mie_dielectric_rcs_dBsm(radius_m, freq, real(eps_r), real(mu_r), θs_obs)
    ...
    r2 = save_and_report_case("P1_PMCHW_Sphere_Direct_phi90_vs_Mie",
        θ_deg_obs, rcs_phi90, mie_phi0, 2.0)  # 球体各向同性
...
if "P3" in enabled
    ...
    radius_m = extract_sphere_radius(mesh_file)
    @printf("  球半径: %.4f m\n", radius_m)
    mie_phi0 = mie_dielectric_rcs_dBsm(radius_m, freq, eps_r, mu_r, θs_obs)
    ...
    r2 = save_and_report_case("P3_PMCHW_LossySphere_Direct_phi90_vs_Mie",
        θ_deg_obs, rcs_phi90, mie_phi0, 2.0)
    push!(all_results, r1, r2)
end
