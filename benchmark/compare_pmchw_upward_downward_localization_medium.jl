...
function near_pairs_basis(octree, sorted_ids)
    ...
    for i_cube = 1:leaf_level.nCubes
...
function direct_m_pass_refs(pmchw, basis, op)
    ...
    hm_k0 = assemble_impedance_matrix(
        efie_from_keta(pmchw.k0, pmchw.eta0, im * pmchw.k0 / (pmchw.eta0 * 16pi)),
        basis,
    )
    hm_k1 = assemble_impedance_matrix(
        efie_from_keta(k1_real, eta1_real, im * pmchw.k1 / (pmchw.eta1 * 16pi)),
        basis,
    )
    ...
    return (k0 = (EM = em_k0, HM = hm_k0), k1 = (EM = em_k1, HM = hm_k1))
end
...
function run_translate_downward!(octree)
    ...
    for levelID = 2:(octree.nLevels-1)
...
function apply_exact_upward_chain!(octree, basis, x_phys, sorted_ids, x_range, k; quad_order = 4)
    ...
    for levelID = (octree.nLevels-1):-1:2
        ...
        exact_parent =
            exact_level_agg(parent_level, basis, x_phys, sorted_ids, x_range, k; quad_order)
...
function leaf_cube_receive_metrics(octree, basis, pmchw, sorted_ids, kmode, x_m, refs)
    ...
    for iCube = 1:leaf_level.nCubes
        ...
        for (local_idx, bfID_sorted) in enumerate(cube.bfInterval)
            ...
            te, tm =
                PMCHWMLFMAOperatorModule._receive_terms(bf, basis, field, r0, k, leaf_level.poles)
        ...
        rel_total =
            norm(vcat(fast_e, fast_h) - vcat(dense_e, dense_h)) /
            (norm(vcat(dense_e, dense_h)) + 1e-30)

        if rel_total > worst_total.rel
            worst_total =
                (cube = iCube, rel = rel_total, rel_e = rel_e, rel_h = rel_h, nbf = length(ids))
        end
    end
...
function leaf_cube_receive_with_rule(
    octree,
    basis,
    pmchw,
    sorted_ids,
    ...
    rel_h = norm(fast_h - dense_h) / (norm(dense_h) + 1e-30)
    rel_total =
        norm(vcat(fast_e, fast_h) - vcat(dense_e, dense_h)) / (norm(vcat(dense_e, dense_h)) + 1e-30)
...
function analyze_case(name, budget, pmchw, basis, x_phys)
    ...
    for kmode in (:k0, :k1)
        ...
        approx_basis_rows = leaf_cube_basis_detail(
            octree,
            basis,
            pmchw,
            sorted_ids,
        ...
        exact_upward_leaf_worst =
            leaf_cube_receive_metrics(octree, basis, pmchw, sorted_ids, kmode, x_m, refs)
        worst_disagg =
            worst_level_disagg_difference(approx_disagg_snapshots, exact_disagg_snapshots)
        cube_leaf_disagg = cube_disagg_difference(
            approx_disagg_snapshots,
            exact_disagg_snapshots,
            octree.nLevels,
            leaf_worst.cube,
        )
        @printf(
        ...
        alt_rule4 = leaf_cube_receive_with_rule(
            octree,
            basis,
            pmchw,
            sorted_ids,
        ...
        alt_rule7 = leaf_cube_receive_with_rule(
            octree,
            basis,
            pmchw,
            sorted_ids,
        ...
        exact3_leaf_worst =
            leaf_cube_receive_metrics(octree, basis, pmchw, sorted_ids, kmode, x_m, refs)
        exact3_cube_leaf_disagg = cube_disagg_difference(
            approx_disagg_snapshots,
            exact3_disagg_snapshots,
            octree.nLevels,
            leaf_worst.cube,
        )
        @printf(
        ...
        field_rows = leaf_cube_field_difference(
            approx_disagg_snapshots,
            exact3_disagg_snapshots,
            octree.nLevels,
            leaf_worst.cube,
