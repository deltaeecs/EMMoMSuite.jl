...
- **Repo hygiene**: 从 git 历史移除非必要二进制文件——`deps/InterpolationWeights/`（约 265 MB HDF5 插值权重缓存，无调用者且可按需重新生成）、历史遗留的 `sphere_mesh_data.jld2`，以及生成的测试产物（`test/test_results/`、`test_results/TestPlate.*`）。
