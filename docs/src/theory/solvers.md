...
*   **原理**：寻找 \$\mathbf{I}_m \in \mathbf{I}_0 + \mathcal{K}_m\$，使得 \$||\mathbf{V} - \mathbf{Z} \mathbf{I}_m||_2\$ 最小。
*   **特点**：
    *   适用于任意非奇异非对称矩阵。
    *   收敛平稳，单调下降。
...
\$\$
\min_{\mathbf{M} \in \mathcal{S}} ||\mathbf{M}\mathbf{Z} - \mathbf{I}||_F^2 = \min_{\mathbf{M} \in \mathcal{S}} \sum_{k=1}^N ||\mathbf{M} \mathbf{z}_k - \mathbf{e}_k||_2^2
\$\$
其中 \$\mathcal{S}\$ 是预先设定的稀疏模式，\$\mathbf{z}_k\$ 是 \$\mathbf{Z}\$ 的第 \$k\$ 列（实际上通常使用 \$\mathbf{Z}\mathbf{M} \approx \mathbf{I}\$ 的形式，即最小化 \$||\mathbf{Z}\mathbf{M} - \mathbf{I}||_F\$，此时问题分解为对 \$\mathbf{M}\$ 的每一列 \$\mathbf{m}_k\$ 的独立最小二乘问题）：
\$\$
\min_{\mathbf{m}_k} ||\mathbf{Z} \mathbf{m}_k - \mathbf{e}_k||_2^2
\$\$
...
4.  **求解 LS 问题**：
    \$\$
    \min_{\hat{\mathbf{m}}_k} ||\hat{\mathbf{Z}}_k \hat{\mathbf{m}}_k - \hat{\mathbf{e}}_k||_2
    \$\$
    通常使用 QR 分解求解此小规模问题。

#### 3.2.3 稀疏模式选择
*   **静态模式 (Static Pattern)**：直接采用 \$\mathbf{Z}\$ (或其稀疏部分 \$\mathbf{Z}_{near}\$) 的稀疏模式，或者其幂次 \$\mathbf{Z}^p\$ 的模式。
*   **动态模式 (SPAI 算法)**：自适应地添加非零元素以降低残差。如果当前残差 \$||\mathbf{Z} \mathbf{m}_k - \mathbf{e}_k||_2\$ 过大，则根据梯度方向寻找新的索引加入 \$\mathcal{J}_k\$，直到满足容差。
