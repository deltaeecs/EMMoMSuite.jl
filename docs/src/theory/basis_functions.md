...
![常见类型网格剖分目标示意图：(a) 三角形面网格剖分球面；(b) 四面体网格剖分圆柱体；(c) 六面体网格剖分圆柱体；(d) 混合网格剖分圆柱体（论文图 2-5）](figures/meshes.png)

对面积分方程用平面三角形剖分目标表面（对应 RWG 基函数），对体积分方程用
四面体（SWG）、六面体（RBF）或混合剖分（PWC 无共形要求）。
...
![各种类型的基函数：(a) RWG；(b) SWG；(c) 屋顶基函数；(d)(e) 分片常数基函数（论文图 2-6）](figures/bfs.png)
