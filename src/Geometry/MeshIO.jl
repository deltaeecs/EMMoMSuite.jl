            ...
            elseif startswith(line, "CTRIA3")
                if occursin(",", line)
                    ...
                    if length(parts) >= 6
                        ...
                        if haskey(grid_id_map, g1) &&
                           haskey(grid_id_map, g2) &&
                           haskey(grid_id_map, g3)
                            triangles[:, tri_idx] =
                                [grid_id_map[g1], grid_id_map[g2], grid_id_map[g3]]
...
function _parse_chexa(line::String, f::IO, grid_id_map::Dict{Int,Int})
    if occursin(",", line)
        ...
        pid_str = parse_nastran_field(line, 17, 24)
        g_strs = [parse_nastran_field(line, 25 + 8 * (k - 1), 32 + 8 * (k - 1)) for k = 1:6]

        # Read continuation line for G7, G8
        if !eof(f)
            cont_line = readline(f)
            if startswith(cont_line, "+") ||
               startswith(strip(cont_line), "+") ||
               !startswith(cont_line, r"\$|GRID|CTRIA|CTETRA|CHEXA|END|BEGIN")
