...
"""
function generate_ellipsoid_mesh(a::FT, b::FT, c::FT, n_theta::Int, n_phi::Int) where {FT<:Real}
...
function generate_cone_mesh(
    ...
    for i = 0:(n_rings-2)
        ...
        for j = 0:(n_circum-1)
            p1 = s_curr + j
            p2 = s_curr + (j + 1) % n_circum
            p3 = s_next + j
            p4 = s_next + (j + 1) % n_circum
            elements[:, eidx] = [p1, p3, p2]
            eidx += 1
            elements[:, eidx] = [p2, p3, p4]
            eidx += 1
        end
    end
...
"""
function generate_torus_mesh(R::FT, r::FT, n_major::Int, n_minor::Int) where {FT<:Real}
...
"""
function generate_box_tet_mesh(Lx::FT, Ly::FT, Lz::FT, nx::Int, ny::Int, nz::Int) where {FT<:Real}
    dx, dy, dz = Lx / nx, Ly / ny, Lz / nz
    ...
    for k = 0:(nz-1), j = 0:(ny-1), i = 0:(nx-1)
        ...
        tetras[:, eidx] = [v[1], v[2], v[3], v[7]]
        eidx += 1  # σ=(1,2,3)
        tetras[:, eidx] = [v[1], v[2], v[7], v[6]]
        eidx += 1  # σ=(1,3,2)*
        tetras[:, eidx] = [v[1], v[4], v[7], v[3]]
        eidx += 1  # σ=(2,1,3)*
        tetras[:, eidx] = [v[1], v[4], v[8], v[7]]
        eidx += 1  # σ=(2,3,1)
        tetras[:, eidx] = [v[1], v[5], v[6], v[7]]
        eidx += 1  # σ=(3,1,2)
        tetras[:, eidx] = [v[1], v[5], v[7], v[8]]
        eidx += 1  # σ=(3,2,1)*
    end
