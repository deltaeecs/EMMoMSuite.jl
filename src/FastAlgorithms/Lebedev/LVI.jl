...
"""
function Interpolation.levelIntegralInfoCal(levelCubeEdgel::FT, ::Val{:LbTrained1Step}; λ=1.0) where{FT<:Real}
    ## 计算截断项
...
带参数的构造函数
"""
function LbTrainedInterp1tepInfo(pk::Int, pt::Int; FT = Float64, depath = joinpath(@__DIR__, "../../../deps/InterpolationWeights/"))
...
Lebedev一步插值
"""
function Interpolation.interpolate(weights::LbTrainedInterp1tepInfo{IT, FT}, data::AbstractArray) where {IT, FT}
...
Lebedev一步反插值
"""
function Interpolation.anterpolate(weights::LbTrainedInterp1tepInfo{IT, FT}, data::AbstractArray) where {IT, FT}
...
Lebedev一步插值
"""
function Interpolation.interpolate!(target::AbstractArray, weights::LbTrainedInterp1tepInfo{IT, FT}, data::AbstractArray) where {IT, FT}
...
Lebedev一步反插值
"""
function Interpolation.anterpolate!(target::AbstractArray, weights::LbTrainedInterp1tepInfo{IT, FT}, data::AbstractArray) where {IT, FT}
    mul!(reshape(target, :), weights.θϕCSCT, reshape(data, :))
    return target
end

function Interpolation.interpolationCSCMatCal(tLevelPoles::LbPolesInfo{FT}, kLevelPoles::LbPolesInfo{FT}, ::IT=8) where {IT<:Integer, FT<:Real}
    # 多极子数
...
function Interpolation.interpolationCSCMatCal(tLevelPoles::GLPolesInfo{FT}, kLevelPoles::LbPolesInfo{FT}, ::IT=8) where {IT<:Integer, FT<:Real}
    # 多极子数
    nk = length(kLevelPoles.Wθϕs)
    # 多项式阶数
    pk = n2pDict[nk]
