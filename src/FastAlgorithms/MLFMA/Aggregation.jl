...
"""
function aggregate!(octree::OctreeInfo, basis::AbstractBasisFunction, operator::AbstractIntegralOperator, x::AbstractVector, sorted_ids::Vector{Int})
    aggregate!(octree, AbstractBasisFunction[basis], [num_basis(basis)], operator, x, sorted_ids)
end

function aggregate!(octree::OctreeInfo, bases::Vector{<:AbstractBasisFunction}, offsets::Vector{Int}, operator::AbstractIntegralOperator, x::AbstractVector, sorted_ids::Vector{Int})
...
"""
function aggregate_leaf!(level::LevelInfo, bases::Vector{<:AbstractBasisFunction}, offsets::Vector{Int}, operator::AbstractIntegralOperator, x::AbstractVector, sorted_ids::Vector{Int})
    ...
    Threads.@threads for iCube in 1:nCubes
        ...
        for i_sorted in start_idx:end_idx
            ...
            if basis isa RWGBasis
                add_radiation_pattern_rwg!(level.aggS, iCube, basis, bf, elem_info, gq, k, cubeCenter, poles_r̂, poles_θhat, poles_ϕhat, coef)
            elseif basis isa SWGBasis
                add_radiation_pattern_swg!(level.aggS, iCube, basis, bf, elem_info, gq, k, cubeCenter, poles_r̂, poles_θhat, poles_ϕhat, coef)
            end
        end
    end
end

function add_radiation_pattern_rwg!(aggS, iCube, basis, bf, elem_info, gq, k, cubeCenter, poles_r̂, poles_θhat, poles_ϕhat, coef)
    JK = im * k
...
function add_radiation_pattern_swg!(aggS, iCube, basis, bf, elem_info, gq, k, cubeCenter, poles_r̂, poles_θhat, poles_ϕhat, coef)
    JK = im * k
    n_qp = length(gq.weight)
    nPoles = length(poles_r̂)
...
function aggregate_upward!(parentLevel::LevelInfo, childLevel::LevelInfo)
    ...
    Threads.@threads for iCube in 1:nCubesParent
