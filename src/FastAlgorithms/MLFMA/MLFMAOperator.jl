...
function assemble_near_field(
    ...
    cube_tets_vec = [Int[] for _ = 1:n_cubes]
    ...
    for i_cube = 1:n_cubes
        ...
        for sorted_idx in cube.bfInterval
            ...
            if b isa RWGBasis
                for k = 1:2
                    if f.support[k] > 0
                        push!(tris_set, f.support[k])
                    end
                end
            elseif b isa SWGBasis
                for k = 1:2
                    if f.support[k] > 0
                        push!(tets_set, f.support[k])
                    end
                end
            end
        end
    ...
    Vs = [Vector{ComplexF64}(undef, estimated_nnz_per_thread) for _ = 1:max_tid]
    ...
    Threads.@threads :static for i_cube = 1:n_cubes
        ...
        for neighbor_idx in neighbors
            ...
            if isempty(neighbor_cube.bfInterval)
                continue
            end
                    ...
                    for t_src in neigh_tris
                        ...
                        if operator isa SCFIE
                            ...
                            @inbounds for idx in eachindex(Z_local)
                                Z_local[idx] =
                                    alpha * Z_local[idx] + (1.0 - alpha) * Z_mfie_buf[idx]
                            end
                        elseif operator isa CFIE
                            ...
                            @inbounds for idx in eachindex(Z_local)
                                Z_local[idx] =
                                    operator.alpha * Z_local[idx] +
                                    (1.0 - operator.alpha) * Z_mfie_buf[idx]
                            end
...
"""
@inline function _ensure_capacity!(
    Is::Vector{Vector{Int}},
    Js::Vector{Vector{Int}},
    Vs::Vector{Vector{ComplexF64}},
    counts::Vector{Int},
    tid::Int,
    needed::Int,
)
...
"""
function distribute_term_nosign!(
    Is,
    Js,
    Vs,
    counts,
