...
"""
function compute_translation_factors!(nLevels::Int, levels::Dict{Int, LV}, k::Real) where{LV<:AbstractLevel}
    ...
    all343InFar316 = OffsetArray(zeros(Int, 2*max_range+1, 2*max_range+1, 2*max_range+1), -max_range:max_range, -max_range:max_range, -max_range:max_range)
    
    indexfar316 = 0
    for kz in -max_range:max_range
        for ky in -max_range:max_range
            for kx in -max_range:max_range
...
function cal_alpha_trans_on_level!(level::LevelInfo, all316FarNeighID::Matrix{Int}, all343InFar316::OffsetArray, k::Real, nFar::Int)
    ...
    Threads.@threads for iFarNei in 1:nFar
...
function translate!(level::LevelInfo)
    ...
    Threads.@threads for iCube in 1:length(cubes)
        cube = cubes[iCube]
