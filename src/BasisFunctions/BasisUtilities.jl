...
export count_unknowns,
    get_triangle_info, get_triangles_info, get_tetrahedra_info, get_hexahedra_info
...
function get_triangle_info(mesh::TriangleMesh{IT,FT}, basis::RWGBasis{IT,FT}, t::Int) where {IT,FT}
    ...
    verts = SMatrix{3,3,FT}(v1[1], v1[2], v1[3], v2[1], v2[2], v2[3], v3[1], v3[2], v3[3])
...
function get_hexahedra_info(
    ...
    hex_bfs = [zeros(IT, 6) for _ = 1:nhex]
    hex_signs = [zeros(Int, 6) for _ = 1:nhex]
    ...
    for i = 1:nhex
        ...
        verts = SMatrix{3,8,FT,24}(hcat([SVector{3,FT}(nodes[:, vid[j]]) for j = 1:8]...))
        ...
        for f = 1:6
            ...
            fv = SMatrix{3,4,FT,12}(hcat([verts[:, fv_ids[j]] for j = 1:4]...))

            # Edge vectors: v1→v2, v2→v3, v3→v4, v4→v1
            ev_cols = [fv[:, mod1(j + 1, 4)] - fv[:, j] for j = 1:4]
            el_vals = [norm(ev_cols[j]) for j = 1:4]
            evu_cols = [ev_cols[j] / el_vals[j] for j = 1:4]
            ...
            en_cols = [cross(face_n_unit, evu_cols[j]) for j = 1:4]
