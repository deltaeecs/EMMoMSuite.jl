...
The table below comes from [test_results/reports/PERFORMANCE_BASELINE.csv](test_results/reports/PERFORMANCE_BASELINE.csv), used by the current stable report. These numbers are baselines on the current validation machine and should not be treated as cross-machine guarantees.
...
The executive summary of the current unified report comes from [test_results/reports/RELEASE_VALIDATION_REPORT.md](test_results/reports/RELEASE_VALIDATION_REPORT.md). As of the latest validation:
